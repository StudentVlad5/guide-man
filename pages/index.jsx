...
export default function HomePage({ questions, news }) {
  ...
  const valuesServiceType = [
    ...
    t('services.lawyersRequests'),
  ];

  const valuesService =
    filterValue.serviceType.length > 0
      ? servicesArray
    ...
    >
      <div className={styles.banner}>
        <div className="container">
          <h2 className={styles.banner__title}>{t('homePage.banner.h1')}</h2>
          <div className={styles.banner__dropdowns}>
            ...
            <div className={`${styles.banner__label} onDesktop`}>
              <div
                className={`${styles.banner__label__title} ${styles.banner__label__title__tulp}`}
              >
                fgds
              </div>
              <button
                className={`button ${styles.banner__button}`}
            ...
            <button
              className={`button ${styles.banner__button} onMobile`}
              onClick={handleTransition}
            >
              <p>{t('homePage.banner.go')}</p>
            </button>
      ...
      <div className={stylesHome.homePage}>
          ...
          <section className="container">
            ...
            <div className={stylesHome.homePage__questions}>
              {questions.slice(0, 5).map(question => (
                <DropdownWithText item={question} key={question.id} />
              ))}
            </div>
          </section>
        </div>
        ...
        <div className="page">
          <section className="container">
            <div
              className={`${stylesHome.homePage__title} page__title-with-extension`}
            >
            ...
            <div className={stylesHome.homePage__explanation}>
              {news.slice(0, 6).map(el => {
                return <NewsItem item={el} key={el.id} isNews={true} />;
              })}

              <button
                className={`button onMobile ${stylesHome.homePage__allButton}`}
              >
                <Link href="/news">
                  <p>{t('newsPage.button')}</p>
                </Link>
